...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.nanoclawbster')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.nanoclawbster'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active nanoclawbster`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('nanoclawbster')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'nanoclawbster', 'mount-allowlist.json'))) {
