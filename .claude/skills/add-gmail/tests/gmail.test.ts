...
function isGmailApplied(): boolean {
  const p = path.join(root, '.nanoclawbster/state.yaml');
  if (!fs.existsSync(p)) return false;
  return read('.nanoclawbster/state.yaml').includes('name: gmail');
}

function getGmailMode(): 'tool-only' | 'channel' {
  const p = path.join(root, '.nanoclawbster/state.yaml');
  if (!fs.existsSync(p)) return 'channel';
  return read('.nanoclawbster/state.yaml').includes('mode: tool-only') ? 'tool-only' : 'channel';
}

const gmailApplied = isGmailApplied();
const mode = getGmailMode();
const channelOnly = mode === 'tool-only';

describe('add-gmail skill', () => {
  it.skipIf(!gmailApplied)('container-runner mounts ~/.gmail-mcp', () => {
    expect(read('src/container-runner.ts')).toContain('.gmail-mcp');
  });

  it.skipIf(!gmailApplied)('agent-runner has gmail MCP server', () => {
  ...
  it.skipIf(!gmailApplied || channelOnly)('gmail channel file exists', () => {
    expect(fs.existsSync(path.join(root, 'src/channels/gmail.ts'))).toBe(true);
  });

  it.skipIf(!gmailApplied || channelOnly)('index.ts wires up GmailChannel', () => {
    expect(read('src/index.ts')).toContain('GmailChannel');
  });

  it.skipIf(!gmailApplied || channelOnly)('googleapis dependency installed', () => {
