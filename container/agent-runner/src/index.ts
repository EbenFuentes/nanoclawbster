...
const SECRET_ENV_VARS = ['ANTHROPIC_API_KEY', 'CLAUDE_CODE_OAUTH_TOKEN', 'COMPOSIO_API_KEY'];
...
/**
 * Write an IPC message file directly (used for startup notifications).
 * This bypasses the MCP layer and writes straight to the messages dir.
 */
function writeIpcMessage(chatJid: string, groupFolder: string, text: string): void {
  ...
  log(`Startup IPC message written: ${filename}`);
}
  ...
  try {
    while (true) {
      ...
      try {
        ...
        // If the session history is too large, clear it and retry with a fresh
        // session so the agent isn't permanently stuck.
        if (msg.includes('Prompt is too long') && sessionId) {
          log(`Prompt too long with session ${sessionId}, retrying with fresh session`);
          sessionId = undefined;
          resumeAt = undefined;
          queryResult = await runQuery(prompt, sessionId, mcpServerPath, containerInput, sdkEnv, resumeAt);
        } else {
